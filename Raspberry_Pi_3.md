...
Configure the `i2c-dev` and `i2c-bcm2835` (if not blacklisted for the camera) modules to be loaded at boot in
...
the previous step with '0x' as prefix (e.g. 0x48) and choose a device name:
    ...
    echo <devicename> <hardware address> >/sys/class/i2c-adapter/i2c-1/new_device

Check `dmesg` for a new entry:
    
    i2c-1: new_device: Instantiated device ds1621 at 0x48
    
Finally, read the sensor output:
    sensors

Default permissions on `i2c` devices are restrictive and require `root` or `sudo` in order to access:
    $ ls -l /dev/i2c-1 
    crw------- 1 root root 89, 1 Aug 16 12:20 /dev/i2c-1

To avoid using these privileges, one may implement [udev rules](https://wiki.archlinux.org/index.php/Udev#Writing_udev_rules) to adjust the permissions. For instance:
